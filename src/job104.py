...
def main():
    """主程式：搜尋 104 人力銀行工作機會並顯示結果。"""
    print("=" * 60)
    print("104 人力銀行工作搜尋")
    print("=" * 60)

    # 範例搜尋：Python 工程師，台北市，薪資 50K 以上
    try:
        ...
        if result.get("status") == 200:
            ...
            print(f"  關鍵字: Python")
            print(f"  地區: 台北市、新北市")
            print(f"  最低薪資: 50,000 元")
            print(f"  發布時間: 7 天內")
            print(f"  週休二日: 是")
            print(f"  排序: 依日期\n")

            print(f"找到 {total} 筆工作，顯示前 {len(jobs)} 筆：")
            print("=" * 60)

            for i, job in enumerate(jobs, 1):
                print(f"\n{i}. {job.get('jobName', 'N/A')}")
                print(f"   公司: {job.get('custName', 'N/A')}")
                print(f"   地區: {job.get('jobAddrNoDesc', 'N/A')}")
                print(f"   薪資: {job.get('salaryDesc', 'N/A')}")
                print(f"   更新: {job.get('appearDateDesc', 'N/A')}")
                print(f"   連結: https://www.104.com.tw/job/{job.get('jobNo', '')}")

            print("\n" + "=" * 60)
            print("搜尋完成！")

        else:
            print(f"\n錯誤: API 回傳狀態碼 {result.get('status')}")
            print(f"訊息: {result.get('errorMsg', 'N/A')}")
